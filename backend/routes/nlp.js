...
// Uses Claude API (Anthropic) to parse user queries into structured filters
...
// ── Parse natural language with Claude API ──────────────────
...
Extract search filters from the user's natural language query and return ONLY a JSON object with these fields:
{
  ...
  "maxBudget": number|null,      // 1=₹(cheap), 2=₹₹(medium), 3=₹₹₹(expensive) — interpret phrases like "cheap", "under 200", "affordable", "budget" as 1; "moderate" as 2; "expensive","premium" as 3
  ...
  return restaurants
    ...
    .filter(r => {
      ...
      if (filters.keywords?.length) {
        ...
        const matched = kws.some(kw => searchable.includes(kw));
      ...
      if (filters.tags?.length) {
        const hasTags = filters.tags.some(t => r.tags?.includes(t));
...
router.post('/search', async (req, res) => {
  try {
    ...
    let nlpUsed  = true;
    try {
      filters = await parseWithClaude(query);
    } catch (e) {
      console.warn('Claude parse failed, falling back to keyword search:', e.message);
...
// Simple keyword search (no NLP) — searches name + menu items
  ...
  try {
    ...
    const results = all
      .map(r => {
        ...
        // Find which menu items matched (for UI display)
      ...
      .filter(r => {
        ...
        const inName     = r.name.toLowerCase().includes(ql);
        const inCuisine  = (r.cuisines || []).some(c => c.toLowerCase().includes(ql));
        const inTags     = (r.tags || []).some(t => t.toLowerCase().includes(ql));
        const inMenu     = r.matchedItems.length > 0;
        return inName || inCuisine || inTags || inMenu;
      })
