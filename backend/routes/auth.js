...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `OTP sent to ${user.email}. It expires in 10 minutes.`
    });
  } catch (err) {
...
router.get('/profile', protect, async (req, res) => {
  try {
    const user = await User.findById(req.user._id)
      .populate('likedRestaurants', 'name emoji address');
